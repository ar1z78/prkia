    ...
    , m_findview(db, settings)
{
	...
	if (StringUtil::to_lower(m_settings->getValue(_T("RecordStats"))) == _T("no"))
	{
...
{
	bool bNewState = !m_infoview.IsWindowVisible();
	m_infoview.ShowWindow(bNewState ? SW_SHOW : SW_HIDE);

	// 1. Sync state in AOManager
	//AOManager::instance().setShowInfo(bNewState);

	// 2. Safely tell the Top Level Window to update the menu
...
LRESULT InventoryView::onDoubleClick(LPNMHDR lParam) {
	...
	ItemListDataModelPtr data_model = std::static_pointer_cast<ItemListDataModel>(m_datagrid->getModel());
...
LRESULT InventoryView::OnColumnClick(LPNMHDR lParam)
{
	...
	{
		// Index 2 is the COL_ITEM_VALUE column
		if (pnmv->iSubItem == 2)
		{
			return 0; // Exit immediately, disabling sort for Price
		}
...
LRESULT InventoryView::OnItemContextMenu(LPNMHDR lParam)
{
    ...
    if (sel >= 0)
    {
		...
		std::tstring n1 = aoia::GetServerName(1);
		std::tstring n2 = aoia::GetServerName(2);
		std::tstring n3 = aoia::GetServerName(3);

		// VIEW STATS Links
		popup.ModifyMenu(ID_VIEW_ITEMSTATS_AUNO, MF_BYCOMMAND | MF_STRING, ID_VIEW_ITEMSTATS_AUNO, n1.c_str());
		popup.ModifyMenu(ID_VIEW_ITEMSTATS_JAYDEE, MF_BYCOMMAND | MF_STRING, ID_VIEW_ITEMSTATS_JAYDEE, n2.c_str());
		popup.ModifyMenu(ID_VIEW_ITEMSTATS_XYPHOS, MF_BYCOMMAND | MF_STRING, ID_VIEW_ITEMSTATS_XYPHOS, n3.c_str());

		// HTML Links
		popup.ModifyMenu(ID_COPY_AUNO_ITEMREF, MF_BYCOMMAND | MF_STRING, ID_COPY_AUNO_ITEMREF, n1.c_str());
		popup.ModifyMenu(ID_COPY_JAYDEE_ITEMREF, MF_BYCOMMAND | MF_STRING, ID_COPY_JAYDEE_ITEMREF, n2.c_str());
		popup.ModifyMenu(ID_COPY_XYPHOS_ITEMREF, MF_BYCOMMAND | MF_STRING, ID_COPY_XYPHOS_ITEMREF, n3.c_str());

		// VBB Links
		popup.ModifyMenu(ID_COPY_AUNO_ITEMREF_VBB, MF_BYCOMMAND | MF_STRING, ID_COPY_AUNO_ITEMREF_VBB, n1.c_str());
		popup.ModifyMenu(ID_COPY_JAYDEE_ITEMREF_VBB, MF_BYCOMMAND | MF_STRING, ID_COPY_JAYDEE_ITEMREF_VBB, n2.c_str());
		popup.ModifyMenu(ID_COPY_XYPHOS_ITEMREF_VBB, MF_BYCOMMAND | MF_STRING, ID_COPY_XYPHOS_ITEMREF_VBB, n3.c_str());

		// CSV Export
		popup.ModifyMenu(ID_EXPORTTOCSV_AUNO, MF_BYCOMMAND | MF_STRING, ID_EXPORTTOCSV_AUNO, n1.c_str());
		popup.ModifyMenu(ID_EXPORTTOCSV_JAYDEE, MF_BYCOMMAND | MF_STRING, ID_EXPORTTOCSV_JAYDEE, n2.c_str());
		popup.ModifyMenu(ID_EXPORTTOCSV_XYPHOS, MF_BYCOMMAND | MF_STRING, ID_EXPORTTOCSV_XYPHOS, n3.c_str());
		
        popup.TrackPopupMenu(TPM_LEFTALIGN | TPM_TOPALIGN, pos.x, pos.y, m_hWnd);
    }
...
LRESULT InventoryView::OnCopyItemRef(WORD FromAccelerator, WORD CommandId, HWND hWndCtrl, BOOL& bHandled)
{
    ...
    case FORMAT_AO:
        ...
        postfix =
            _T(
            "<p><p><center>Generated by <a href='chatcmd:///start https://users.math.uoc.gr/~ar1z/prkia.html'>PRKIA</a></center>\">view</a>");
        separator = _T("<br>");
        break;
	...
	StringUtil::replace_all(itemTemplate, _T("%url%"), urlTemplate);

    ItemListDataModelPtr model = std::static_pointer_cast<ItemListDataModel>(m_datagrid->getModel());
    ...
    for (std::set<unsigned int>::iterator it = selectedIndexes.begin(); it != selectedIndexes.end(); ++it)
    {
        ...
        StringUtil::replace_all(itemlocation, _T("\""), _T("&quot;"));

        std::tstring itemStr = itemTemplate;
        StringUtil::replace_all(itemStr, _T("%lowid%"), pItemInfo->itemloid);
        StringUtil::replace_all(itemStr, _T("%hiid%"), pItemInfo->itemhiid);
        StringUtil::replace_all(itemStr, _T("%ql%"), pItemInfo->itemql);
        StringUtil::replace_all(itemStr, _T("%itemname%"), pItemInfo->itemname);
        StringUtil::replace_all(itemStr, _T("%itemlocation%"), itemlocation);
	...
	if (scriptOutput)
	{
		...
		{
			// 1. Get the Prefs folder (e.g., ...\AccountName\CharID\Prefs)
			std::tstring prefsFolder = AOManager::instance().getAOPrefsFolder();
			if (prefsFolder.empty()) return 0;

			// 2. Go up one level to get the character folder (...\AccountName\CharID)
			size_t last_slash = prefsFolder.find_last_of(_T("\\/"));
			std::tstring charFolder = (last_slash == std::string::npos) ? prefsFolder : prefsFolder.substr(0, last_slash);

			// 3. Construct the script path: ...\AccountName\CharID\scripts\aoia
			std::tstring scriptFile = charFolder;
			scriptFile.append(_T("\\scripts\\aoia"));

			// 4. Ensure the 'scripts' directory exists (Windows 10 safety)
			std::tstring scriptsDir = charFolder + _T("\\scripts");
			CreateDirectory(scriptsDir.c_str(), NULL);

#ifdef UNICODE
			std::wofstream ofs(scriptFile.c_str());
#else
			std::ofstream ofs(scriptFile.c_str());
#endif
			ofs << output;
...
LRESULT InventoryView::OnShowItemRef(WORD FromAccelerator, WORD CommandId, HWND hWndCtrl, BOOL& bHandled)
{
	...
	// Map the CommandId to your ItemServer enum
	if (CommandId == ID_VIEW_ITEMSTATS_AUNO)
	{
		url = GetServerItemURLTemplate(SERVER_AUNO);
	}
	else if (CommandId == ID_VIEW_ITEMSTATS_JAYDEE)
	{
		url = GetServerItemURLTemplate(SERVER_JAYDEE);
	}
	else if (CommandId == ID_VIEW_ITEMSTATS_XYPHOS)
	{
		url = GetServerItemURLTemplate(SERVER_XYPHOS);
	}
	else
	{
		assert(false);
	...
	if (!url.empty())
	{
		StringUtil::replace_all(url, _T("%lowid%"), STREAM2STR(pItemInfo->itemloid));
		StringUtil::replace_all(url, _T("%hiid%"), STREAM2STR(pItemInfo->itemhiid));
		StringUtil::replace_all(url, _T("%ql%"), STREAM2STR(pItemInfo->itemql));
...
void InventoryView::UpdateListView(std::tstring const& where)
{
    ...
    m_datagridmodel.reset(new ItemListDataModel(m_db, m_containerManager, m_settings, where, m_sortColumn, !m_sortDesc));

	m_addSignalConnection = m_datagridmodel->connectItemAdded([this](unsigned int p1) { onItemAdded(p1); });
	m_removeSignalConnection = m_datagridmodel->connectItemRemoved([this](unsigned int p1) { onItemRemoved(p1); });
	m_clearSignalConnection = m_datagridmodel->connectAllRemoved([this]() { onAllItemsRemoved(); });
	m_updateSignalConnection = m_datagridmodel->connectCollectionUpdated([this]() { onAllItemsUpdated(); });
...
std::tstring InventoryView::ensureEncapsulation(std::tstring const& field)
{
    ...
    if (!StringUtil::starts_with(field, _T("\"")))
    {
        retval = _T("\"");
    }
    ...
    if (!StringUtil::ends_with(field, _T("\"")))
    {
        retval += _T("\"");
    }
...
void InventoryView::exportToCSV(std::tstring const& where)
{
	...
	ItemListDataModelPtr data(new ItemListDataModel(m_db, m_containerManager, m_settings, where_statement, m_sortColumn, !m_sortDesc));
...
{
	std::tstring settingKey;
	UINT resourceId = 0;
	// Map the server to a Setting Key and a Resource ID
	switch (server)
	{
	case SERVER_AUNO:
		settingKey = _T("itemurl1");
		resourceId = IDS_AUNO_ITEMREF_URL;
		break;
	case SERVER_JAYDEE:
		settingKey = _T("itemurl2");
		resourceId = IDS_JAYDEE_ITEMREF_URL;
		break;
	case SERVER_XYPHOS:
		settingKey = _T("itemurl3");
		resourceId = IDS_XYPHOS_ITEMREF_URL;
		break;
	case SERVER_AO:
		return _T("itemref://%lowid%/%hiid%/%ql%"); // Internal - no setting needed
	default:
		return _T("");
	}

	// Access settings via the Manager
	aoia::ISettingsPtr settings = AOManager::instance().getSettings();
	if (!settingKey.empty() && settings)
	{
		std::tstring url = settings->getValue(settingKey);
		if (!url.empty()) {
			return url;
		}
	}

	// Fallback to Resource (and save if external)
	TCHAR buf[512];
	if (::LoadString(_Module.GetResourceInstance(), resourceId, buf, 512))
	{
		std::tstring url = buf;
		if (!settingKey.empty() && settings) {
			settings->setValue(settingKey, url);
		}
		return url;
	}
...
{
    // Set status text
    std::tstring status = STREAM2STR(m_datagrid->getSelectedCount() << _T(" of ") 
                                     << m_datagrid->getModel()->getItemCount() 
                                     << _T(" items selected."));
...
{
    unsigned int containerid = (unsigned int)_ttoi(pItemInfo->containerid.c_str());
    unsigned int ownerid = (unsigned int)_ttoi(pItemInfo->ownerid.c_str());
