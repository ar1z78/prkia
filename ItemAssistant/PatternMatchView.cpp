...
PatternMatchView::PatternMatchView(sqlite::IDBPtr db, aoia::IContainerManagerPtr containerManager, aoia::IGuiServicesPtr gui, aoia::ISettingsPtr settings)
    ...
    , m_webview( _T("") /*STREAM2STR("https://www.ao-universe.com/guides/shadowlands/tradeskill-guides-5/general-crafting-4/pocket-boss-guide") */)
...
LRESULT PatternMatchView::OnCreate(LPCREATESTRUCT createStruct)
{
	...
	m_filterPanel.connectSettingsChanged([this]() { onFilterSettingsChanged(); });
	...
	m_webview.SetHTML(_T("<html><body>Visit https://prk.tinkeringidiot.com/pocket?tab=bosses for more information.</body></html>"));
    ...
    if (pT != NULL)
    {
		...
		{
			auto pPb = std::make_shared<PbItem>();

			pPb->pbid = std::stoul(pT->Data(row, 0));
