        ...
        std::tstring CharacterSummaryFormatter::toString() const
        {
            ...
            for (unsigned int i = 0; i < m_model->getItemCount(); ++i)
            {
                ...
                for (unsigned int j = 0; j < m_model->getColumnCount(); ++j)
                {
                    ...
                    {
                        // Removed try block - _ttoi is safe
                        unsigned int value = (unsigned int)_ttoi(propertyValue.c_str());
                        ...
                        {
                            if (value == 15)      out << 4;
                            else if (value == 7)  out << 3;
                            else if (value == 3)  out << 2;
                            else if (value == 1)  out << 1;
                            else                  out << "-";
                        }
                        ...
                        switch(j)
                        {
                            case 1: totalLevels += value;   break;
                            case 2: totalAiLevels += value; break;
                            case 3: totalCredits += value;  break;
                        }
                    }
                    else
                    {
            ...
            for (unsigned int j = 0; j < m_model->getColumnCount(); ++j)
            {
                ...
                {
                    out << "<td></td>"; // Fill empty cells for SUM row alignment
                }
            }
            out << "</tr>";
            out << "</table>";
