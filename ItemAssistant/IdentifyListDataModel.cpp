...
namespace aoia
{
	...
	{
		if (!m_result) { return _T(""); }

		if (index == 0) { return _T("Item"); }
		if (index == 1) { return _T("Purpose"); }
	...
	{
		if (!m_result || index >= getItemCount()) { return _T(""); }
		return from_ascii_copy(m_result->Data(index, column));
	}

	unsigned int IdentifyListDataModel::getItemId(unsigned int index) const
	{
		if (!m_result || index >= getItemCount()) { return 0; }
		// The AOID is selected as column index 2 in the query
		return (unsigned int)atoi(m_result->Data(index, 2).c_str());
	}

	unsigned int IdentifyListDataModel::getItemLowId(unsigned int index) const
	{
		if (!m_result || index >= getItemCount()) { return 0; }
		// The LOWID is selected as column index 3 in the query
		return (unsigned int)atoi(m_result->Data(index, 3).c_str());
	}

	unsigned int IdentifyListDataModel::getItemHighId(unsigned int index) const
	{
		if (!m_result || index >= getItemCount()) { return 0; }
		// The HIGHID is selected as column index 4 in the query
		return (unsigned int)atoi(m_result->Data(index, 4).c_str());
	}
}
