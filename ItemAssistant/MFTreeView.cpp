...
LRESULT MFTreeView::OnSettingChange(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	Invalidate();
	return 0;
}

void MFTreeView::OnTimer(UINT_PTR nIDEvent)
{
	if (nIDEvent == 101)
	{
		Invalidate();
...
LRESULT MFTreeView::OnGetDispInfo(int, LPNMHDR lParam, BOOL&)
{
   ...
   // Use only a simple NULL check. 
   // If pItem is 0xfeeefeee (deleted), this logic should still catch it in Debug.
   if (pItem == NULL)
   {
      return 0;
   }
	...
	try {
		if (lptvdi->item.mask & TVIF_TEXT)
		{
			// 3. Capture label safely
			std::tstring label = pItem->GetLabel();
			_tcsncpy(lptvdi->item.pszText, label.c_str(), lptvdi->item.cchTextMax);
			lptvdi->item.pszText[lptvdi->item.cchTextMax - 1] = _T('\0');
		}
		...
		if (lptvdi->item.mask & TVIF_TEXT) {
			_tcscpy(lptvdi->item.pszText, _T("Error"));
		}
	}
