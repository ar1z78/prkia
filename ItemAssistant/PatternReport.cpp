    ...
    , m_containerManager(containerManager)
{
    ...
    {  // Find all patternpieces
        ...
        //std::map<std::tstring, unsigned int> PatternItemIds = bost::assign::map_list_of("A", 0)("B", 0)("C", 0)("D", 0)("AB", 0)("ABC", 0)("ABCD", 0);
            ...
            it != PatternItemIds.end(); ++it)
        {
            ...
            {
				unsigned int containerid = std::stoul(pItems->Data(itemIdx, 0));
				unsigned int owner = m_toonid > 0 ? m_toonid : std::stoul(pItems->Data(itemIdx, 1));
...
std::tstring PatternReport::toString() const
{
    ...
    if (pData != NULL)
    {
		...
		//std::string mpbid = std::to_string(m_pbid);
		
		StringUtil::replace_all(html, _T("%TIME_AND_DATE%"), m_time);
		StringUtil::replace_all(html, _T("%TOON_NAME%"), m_toonname);
		StringUtil::replace_all(html, _T("%PB_NAME%"), m_pbname);
		StringUtil::replace_all(html, _T("%PB_AVAILABILITY%"), m_avail);
		StringUtil::replace_all(html, _T("%RESULT_TABLE%"), m_table);
		//StringUtil::replace_all(html, _T("%PB_ID%"), mpbid);
