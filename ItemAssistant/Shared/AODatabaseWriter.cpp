...
// Define a standard static array
static const char* raw_sql[] = {
	...
	"INSERT OR REPLACE INTO tblPocketBoss (pbid, ql, name) SELECT aoid, ql, RTRIM(SUBSTR(name, 34), '''') AS name FROM tblAO WHERE name LIKE '%Novictalized Notum Crystal with %' AND aoid != 232267 ORDER BY aoid",
...
const std::vector<std::string> c_datatransformation_sql(std::begin(raw_sql), std::end(raw_sql));
...
void AODatabaseWriter::WriteItem(std::shared_ptr<ao_item> item)
{
    static std::map<unsigned int, std::string> s_ItemTypeMap = {
        {AODB_ITEM_MISC,    "Misc"},
	...
	StringUtil::replace_all_ansi(name, "'", "''");

	// Prepare and escape the Description
	std::string desc = item->description;
	StringUtil::replace_all_ansi(desc, "'", "''");

	// 3. Build the SQL
	std::ostringstream sql;
	sql << "INSERT INTO tblAO (aoid, name, ql, type, islot, description, flags, properties, icon, ivalue) VALUES ("
		<< item->aoid << ", "
		<< "'" << name << "', "
		<< item->ql << ", "
...
void AODatabaseWriter::writeEffect(unsigned int aoid, ao_item_effect const& eff)
{
    ...
    StringUtil::replace_all_ansi(text, "'", "''");
