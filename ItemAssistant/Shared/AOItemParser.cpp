...
static std::map<unsigned short, unsigned char> s_effectkeys = {
	{
...
AOItemParser::AOItemParser(char* pBuffer, unsigned int bufSize, unsigned int actualId)
{
    this->aoid = actualId; //*((unsigned int*)pBuffer);
...
char* AOItemParser::ParseFunctions(char* pBuffer, unsigned int bufSize, unsigned int ftype, unsigned int key)
{
    ...
    case 0x1b : // upload nano [id]
		...
		if (this->name.find("Nano Crystal") != std::string::npos ||
			this->name.find("NanoCrystal") != std::string::npos ||
			this->name.find(": Startup Crystal - ") != std::string::npos)
        {
