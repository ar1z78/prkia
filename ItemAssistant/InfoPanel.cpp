...
LRESULT InfoView::OnButtonClicked(WORD commandID, WORD buttonID, HWND hButton, BOOL& bHandled)
{
    ...
    g_DBManager.UnLock();
	StringUtil::replace_all(url, _T("%lowid%"), pItemInfo->itemloid);
	StringUtil::replace_all(url, _T("%hiid%"), pItemInfo->itemhiid);
	StringUtil::replace_all(url, _T("%ql%"), pItemInfo->itemql);
