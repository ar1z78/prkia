...
{
	// 1. Safety check for empty folder name
	if (m_folder.empty()) {
		return;
	}

	// It returns TRUE if successful, or FALSE if it fails (e.g., already exists)
	if (!CreateDirectory(m_folder.c_str(), NULL))
	{
		DWORD error = GetLastError();
		// If the error is 'Already Exists', that's fine, we don't need to log it
		if (error != ERROR_ALREADY_EXISTS)
		{
			// 3. Use standard string formatting for the log
			std::tstringstream ss;
			ss << _T("Unable to create directory ") << m_folder
				<< _T(" (Error Code: ") << error << _T(")");
			LOG(to_ascii_copy(ss.str()).c_str());
		}
	}
}
...
{
	std::tstring fullPath = m_folder + _T("\\") + file_name;

	// 2. Open the stream using the .c_str() of our new path
	// ALWAYS use std::ofstream for binary/byte data
	// Even in Unicode mode, we use the wide-path constructor for the filename,
	// but the stream itself remains a narrow byte-stream.
