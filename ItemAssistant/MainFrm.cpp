    ...
    : m_settings(settings)
{
    ...
    std::tstring left = m_settings->getValue(_T("Window.Left"));
    std::tstring top = m_settings->getValue(_T("Window.Top"));
    std::tstring width = m_settings->getValue(_T("Window.Width"));
    std::tstring height = m_settings->getValue(_T("Window.Height"));

    // If any values are missing, skip the intersection test and use defaults
    if (!left.empty() && !top.empty() && !width.empty() && !height.empty())
    {
        m_windowRect.left = _ttoi(left.c_str());
        m_windowRect.top = _ttoi(top.c_str());
        m_windowRect.right = m_windowRect.left + (unsigned int)_ttoi(width.c_str());
        m_windowRect.bottom = m_windowRect.top + (unsigned int)_ttoi(height.c_str());

        Desktop desktop;
        if (!desktop.IntersectsRect(m_windowRect))
        {
            LOG(_T("Previous window not inside current desktop. Using default."));
            m_windowRect.SetRectEmpty();
        }
    }
    else
    {
        LOG(_T("Could not load previous window position and size. Using default."));
        m_windowRect.SetRectEmpty();
    }
}
...
BOOL CMainFrame::OnIdle()
{
	// 1. Get the current active tab index (0=Summary, 1=Inventory, etc.)
	int activeTab = m_tabbedChildWindow->GetTabCtrl().GetCurSel();

	// 2. Determine if the Inventory tab is active
	bool isInventoryActive = (activeTab == 1);

	// 3. Disable or Enable the specific menu items
	// This will gray them out and prevent them from being clicked
	UIEnable(ID_INV_FIND_TOGGLE, isInventoryActive);
...
LRESULT CMainFrame::OnSellerStats(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	CSellerStatsDlg dlg;

	// 1. Fetch the entire settings struct from the manager
	SellerSettings s = AOManager::instance().getSellerSettings();

	// 2. Pass the settings (and the pointer) to the dialog
	dlg.m_settings = m_settings;
	dlg.m_cl = s.clSkill;
	dlg.m_omni = s.isOmni;
	dlg.m_trader = s.isTrader;
	dlg.m_colorLow = s.colorLow;
	dlg.m_colorMed = s.colorMed;
	dlg.m_colorHigh = s.colorHigh;
	dlg.m_priceMax = s.priceMax;

	if (dlg.DoModal() == IDOK) {
		// 3. Re-bundle the modified values into a struct
		SellerSettings newSettings;
		newSettings.clSkill = dlg.m_cl;
		newSettings.isOmni = dlg.m_omni;
		newSettings.isTrader = dlg.m_trader;
		newSettings.colorLow = dlg.m_colorLow;
		newSettings.colorMed = dlg.m_colorMed;
		newSettings.colorHigh = dlg.m_colorHigh;
		newSettings.priceMax = dlg.m_priceMax;


		// 5. If Inventory is active, force a refresh of the pricing math
		// You can send a message or call a refresh if you have a handle to the active view
	}
	return 0;
}
...
LRESULT CMainFrame::OnCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
    ...
    m_trayIcon = std::shared_ptr<CTrayNotifyIcon>(new CTrayNotifyIcon());
    m_trayIcon->Create(this, IDR_TRAY_POPUP, _T("PRK Item Assistant"), hIconSmall, WM_TRAYICON);
	...
	//bool bShowFind = AOManager::instance().getFindToggle();
	//UISetCheck(ID_INV_FIND_TOGGLE, bShowFind);

	bool bRecordStats = AOManager::instance().getRecordStats();
	UISetCheck(ID_RECORD_STATS_TOGGLE, bRecordStats);
...
LRESULT CMainFrame::OnHelp(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	// m_guiServices->OpenURL(_T("file:\\help\help.html"));
    return 0;
}
...
{
    m_guiServices->OpenURL(STREAM2STR(_T("https://users.math.uoc.gr/~ar1z/prkia.html")));
    return 0;
}
...
{
    m_guiServices->OpenURL(_T("https://users.math.uoc.gr/~ar1z/prkia.html"));
    return 0;
}
...
LRESULT CMainFrame::OnUpdateMenuCheck(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
{
    // Use != 0 to fix the C4800 warning
